...
type Versions struct {
	...
	// cloudbreak
	// Read Only: true
	Cloudbreak []*CloudbreakVersion `json:"cloudbreak"`

	// freeipa
	// Read Only: true
	Freeipa []*CloudbreakVersion `json:"freeipa"`
}
...
func (m *Versions) Validate(formats strfmt.Registry) error {
	...
	if err := m.validateCloudbreak(formats); err != nil {
		res = append(res, err)
	}
...
func (m *Versions) validateCloudbreak(formats strfmt.Registry) error {

	if swag.IsZero(m.Cloudbreak) { // not required
		return nil
	}

	for i := 0; i < len(m.Cloudbreak); i++ {
		if swag.IsZero(m.Cloudbreak[i]) { // not required
			continue
		}

		if m.Cloudbreak[i] != nil {
			if err := m.Cloudbreak[i].Validate(formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					return ve.ValidateName("cloudbreak" + "." + strconv.Itoa(i))
				}
				return err
			}
		}
