...
```golang
import model "github.com/GIT_USER_ID/GIT_REPO_ID/model"
```
...
```golang
ctx := context.WithValue(context.Background(), model.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), model.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), model.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), model.ContextOperationServerVariables, map[string]map[string]string{
