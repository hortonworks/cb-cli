...
var getBlueprintClient = func(server, userName, password, tenant, authType string) blueprint.GetBlueprintInWorkspace {
	cbClient := oauth.NewCloudbreakHTTPClient(server, userName, password, tenant, authType)
	return cbClient.Cloudbreak.V3WorkspaceIDBlueprints
}

var stackClient = func(server, userName, password, tenant, authType string) getStackInWorkspace {
	cbClient := oauth.NewCloudbreakHTTPClient(server, userName, password, tenant, authType)
...
func generateStackTemplateImpl(mode cloud.NetworkMode, stringFinder func(string) string, boolFinder func(string) bool, int64Finder func(string) int64, getBlueprintClient func(string, string, string, string, string) blueprint.GetBlueprintInWorkspace, storageType cloud.CloudStorageType) *model.StackV2Request {
	...
	if bpName := stringFinder(fl.FlBlueprintNameOptional.Name); len(bpName) != 0 {
		...
		bpResp := blueprint.FetchBlueprint(workspace, bpName, getBlueprintClient(stringFinder(fl.FlServerOptional.Name), stringFinder(fl.FlUsername.Name), stringFinder(fl.FlPassword.Name), stringFinder(fl.FlTenant.Name), stringFinder(fl.FlAuthTypeOptional.Name)))
...
func generateAttachedTemplateImpl(stringFinder func(string) string, boolFinder func(string) bool, int64Finder func(string) int64, storageType cloud.CloudStorageType) error {
	datalake := fetchStack(int64Finder(fl.FlWorkspaceOptional.Name), stringFinder(fl.FlWithSourceCluster.Name), stackClient(stringFinder(fl.FlServerOptional.Name), stringFinder(fl.FlUsername.Name), stringFinder(fl.FlPassword.Name), stringFinder(fl.FlTenant.Name), stringFinder(fl.FlAuthTypeOptional.Name)))
...
func GenerateReinstallTemplate(c *cli.Context) {
	...
	stackResp := fetchStack(workspaceID, stackName, stackClient(c.String(fl.FlServerOptional.Name), c.String(fl.FlUsername.Name), c.String(fl.FlPassword.Name), c.String(fl.FlTenant.Name), c.String(fl.FlAuthTypeOptional.Name)))
	...
	bpResp := blueprint.FetchBlueprint(workspace, bpName, getBlueprintClient(c.String(fl.FlServerOptional.Name), c.String(fl.FlUsername.Name), c.String(fl.FlPassword.Name), c.String(fl.FlTenant.Name), c.String(fl.FlAuthTypeOptional.Name)))
	bp, err := base64.StdEncoding.DecodeString(bpResp.AmbariBlueprint)
	if err != nil {
		utils.LogErrorAndExit(err)
	}
