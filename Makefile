...
generate-swagger-autoscale:
	swagger generate client -f http://$(CB_IP):8085/as/api/swagger.json -c client_autoscale -m models_autoscale
...
generate-swagger-autoscale-docker:
	...
	generate client -f http://$(CB_IP):8085/as/api/swagger.json -c client_autoscale -m models_autoscale
